...
__all__ = ['fpn_osnet_x1_0', 'fpn_osnet_x0_5',]
...
class OSNetFPN(OSNet):
    ...
    def __init__(self, num_classes, blocks, layers, channels,
        ...
        if self.loss not in ['am_softmax', ]:
            ...
            from .am_softmax import AngleSimpleLinear
    ...
    def featuremaps(self, x):
        ...
        #print('in_first: {} ms'.format((time.time()-prev_total)*1000))
        #prev = time.time()
        x = self.conv1(x)
        x1 = self.maxpool(x)
        #torch.cuda.synchronize()
        #print('conv1 + maxpool: {} ms'.format((time.time()-prev)*1000))
        out.append(x1)
        #prev = time.time()
        x2 = self.conv2(x1)
        #torch.cuda.synchronize()
        #print('conv2: {} ms'.format((time.time()-prev)*1000))
        out.append(x2)
        #prev = time.time()
        x3 = self.conv3(x2)
        #torch.cuda.synchronize()
        #print('conv3: {} ms'.format((time.time()-prev)*1000))
        out.append(x3)
        #prev = time.time()
        x4 = self.conv4(x3)
        x5 = self.conv5(x4)
        #torch.cuda.synchronize()
        #print('conv4 + conv5: {} ms'.format((time.time()-prev)*1000))
        out.append(x5)
        #torch.cuda.synchronize()
        #print('total: {} ms'.format((time.time()-prev_total)*1000))
        return x5, out
    ...
    def forward(self, x):
        ...
        #torch.cuda.synchronize()
        #print('feature maps: {} ms'.format((time.time()-prev)*1000))
        # end.record()
        # print('featureamaps:',start.elapsed_time(end))


        if self.fpn is not None:
            # prev = time.time()
            feature_pyramid = self.fpn(feature_pyramid) # takes around 2.1 ms
            # torch.cuda.synchronize()
            # print('fpn: {} ms'.format((time.time()-prev)*1000))
        ...
        v = v.view(v.size(0), -1)
        #torch.cuda.synchronize()
        #print('all post: {} ms'.format((time.time()-prev)*1000))

        return v


    def process_feature_pyramid(self, feature_pyramid): # takes around 1.3 ms
        # prev = time.time()
        ...
        # torch.cuda.synchronize()
        # print('fpn proc: {} ms'.format((time.time()-prev)*1000))
        return output
